...
void _generate_subgraph(Context & C, ByteInterval & interval, int src, int dst, gtirb::EdgeType type){
    static std::map<int, CodeBlock *> blocks;
    if (!blocks.find(src)){
        auto * block = CodeBlock::Create(C, src) 
        blocks[src] = block;
        block->setSize(1);
    ...
    auto b1 = blocks[src];
    if (!blocks.find(dst)){
        auto * block = CodeBlock::Create(C, dst) 
        blocks[dst] = block;
        block->setSize(1);
